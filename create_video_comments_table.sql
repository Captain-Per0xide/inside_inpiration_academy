...
-- Add foreign key constraint to courses table (optional - skip if courses table doesn't exist)
-- Uncomment the following lines only if your courses table exists and has an 'id' column
-- DO $$
-- BEGIN
--     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'courses') THEN
--         ALTER TABLE video_comments 
--         ADD CONSTRAINT fk_video_comments_course 
...
-- Policy to allow users to update only their own comments
-- Note: This will require storing user_id in the comments JSONB array
